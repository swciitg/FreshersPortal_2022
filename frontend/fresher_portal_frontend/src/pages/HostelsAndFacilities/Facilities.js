...
const Facilities = () => {
  const [nav, setNav] = React.useState({page:0,articles:["Guesthouse","Library","Hospital"]})

  const arr = ["Guesthouse","Library","Hospital","Computer Centre","Shopping Complex","Eateries","Banks & ATMs","Bus Facilitiy","E-Rickshaws"];
...
function LeftArrow(nav,handlePrevClick) {  
    ...
    <Tooltip title="Left" placement="left">
      <Button disabled={nav.page<1} onClick={() => handlePrevClick()}> &lt;- </Button>
    </Tooltip>
    );
  }
  ...
  function RightArrow(nav,handleNextClick) { 
    ...
    <Tooltip title="Right" placement="right">
       <Button disabled={nav.page>1} onClick={() => handleNextClick()}>-&gt;</Button>
     </Tooltip>
    );
  }
